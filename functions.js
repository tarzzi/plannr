...
function renderTasks(project){
    ...
    tasks.forEach(task => {
        if(task.project == project || project == null){
            ...
            tasksList.append(`
            <div class="task-box ${prio_color}" id="${task.id}">
                    <div class="task-name">
                        <span>${task.name}</span>
...
function renderProjects(){
    ...
    $('#projects-list').html('');
    $('#projects-list').append(`<button class="project-item" onclick="renderTasks()">All projects</button>`);
    for(let i = 0; i < projects.length; i++){
        $('#projects-list').append(`<button class="project-item" onclick="renderTasks('${projects[i].name}')">${projects[i].name}</button>`);
    }
}
...
function initStorage(){
    ...
    console.log(tasks);
    projects = (localS('get', 'projects', projects) === null) ? [] : localS('get', 'projects');
    renderTasks();
    renderProjects();
}
...
function localS(option, item, data){
    if(option === "set"){ localStorage.setItem(item, JSON.stringify(data))}
    else{ 
        return (item === "tasks") ? JSON.parse(localStorage.getItem(item)) : JSON.parse(localStorage.getItem(item));
    }
